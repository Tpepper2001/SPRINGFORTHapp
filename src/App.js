...
import { Download, Receipt, Share2, RefreshCw } from 'lucide-react';
...
const EReceiptApp = () => {
  ...
  const downloadReceiptAsPDF = () => {
    ...
    doc.text('SPRINGFORTH ACADEMY', 105, 15, null, null, 'center');
  ...
  const shareReceiptOptions = () => {
    const message = `Springforth Academy Receipt\nReceipt No: ${receiptData.receiptNumber}\nDate: ${receiptData.date}\nFrom: ${receiptData.receivedFrom}\nAmount: ₦${formatAmount(receiptData.amountNumbers)}\nPayment For: ${receiptData.paymentFor || 'Payment received'}`;
    ...
    <div className="p-4 max-w-xl mx-auto">
      ...
      <div className="space-y-3">
        ...
        {receiptData.receiptNumber && receiptData.receivedFrom && receiptData.amountNumbers && (
          <button
            onClick={shareReceiptOptions}
            className="w-full bg-purple-700 text-white px-4 py-2 rounded flex items-center justify-center gap-2"
          >
